...
export default function SupplierInvoicePage() {
  ...
  const handleExportPdf = () => {
    if (invoice && pdfRef.current) {
      exportToPdf(pdfRef.current, `Invoice_${invoice.id}.pdf`);
    }
  };
  ...
  const handleDownloadPdf = () => {
    ...
    setTimeout(async () => {
      if (pdfDivRef.current && invoice) {
        await exportToPdf(
          pdfDivRef.current,
          `Supplier-Invoice-${invoice.id}.pdf`
        );
        ...
        {isPdfOpen &&
          ...
          createPortal(
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
              <div className="bg-white rounded-lg overflow-hidden shadow-xl max-w-3xl w-full max-h-[90vh] p-6">
                ...
                <div ref={pdfRef} className="mb-4">
                  <InvoicePdfView
                    invoice={{
                      ...invoice,
                      clientName: invoice.supplierName, // For PDF view compatibility
                    }}
                  />
                </div>
